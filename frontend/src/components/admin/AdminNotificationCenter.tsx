...
const AdminNotificationCenter: React.FC = () => {
    ...
    const { data: logs = [], isLoading: logsLoading, refetch } = useAdminNotificationLogs();
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (formState.scope === 'custom' && (!customUserIds?.length && !customEmails?.length)) {
            setFeedback({ type: 'error', message: 'Provide at least one user ID or email for custom notifications.' });
            return;
        }
        ...
        try {
            ...
            setFeedback({
                type: 'error',
                message: apiMessage || 'Unable to send the notification. Please try again.',
            });
        }
    };
    ...
    return (
        <div className="tw-grid tw-grid-cols-1 xl:tw-grid-cols-2 tw-gap-6">
            <section className="tw-bg-white tw-border tw-border-gray-200 tw-rounded-xl tw-p-6 tw-shadow-sm" aria-label="Compose admin notification">
                <header className="tw-mb-6">
                    <p className="tw-text-xs tw-uppercase tw-font-semibold tw-text-indigo-500">Broadcast</p>
                    <h2 className="tw-text-2xl tw-font-bold tw-text-gray-900">Compose notification</h2>
                    <p className="tw-text-sm tw-text-gray-600">
                        Notify everyone about announcements, or target mentors, mentees, and custom recipients for match and session alerts.
                    </p>
                </header>
                    ...
                    <fieldset>
                        ...
                        <div className="tw-mt-4">
                            ...
                            <textarea
                                ...
                                disabled={publishDisabled}
                                className="tw-mt-1 tw-w-full tw-rounded-lg tw-border tw-border-gray-300 tw-px-3 tw-py-2 focus:tw-ring-2 focus:tw-ring-indigo-500 focus:tw-border-transparent"
                                placeholder="Defaults to the notification message if left blank."
                            />
                        </div>
                    </fieldset>
            ...
            <section className="tw-bg-white tw-border tw-border-gray-200 tw-rounded-xl tw-p-6 tw-shadow-sm" aria-label="Notification history">
                <header className="tw-mb-6 tw-flex tw-items-center tw-justify-between">
                    <div>
                        ...
                        <h2 className="tw-text-2xl tw-font-bold tw-text-gray-900">Recent notifications</h2>
                    </div>
                    <button
                        type="button"
                        onClick={() => refetch()}
                ...
                {logsLoading && (
                    <div className="tw-text-sm tw-text-gray-500">Loading latest activity…</div>
                )}

                {!logsLoading && trimmedLogs.length === 0 && (
                    <div className="tw-text-sm tw-text-gray-500">No notifications have been sent yet.</div>
                )}
