...
const statusColors: Record<string, string> = {
  ...
  mentor_declined: 'tw-bg-red-50 tw-text-red-700',
  connected: 'tw-bg-emerald-50 tw-text-emerald-700',
};

const MentorMatchesList: React.FC<Props> = ({ matches, isLoading, meta }) => {
  if (isLoading) {
    return <p className="tw-text-sm tw-text-gray-500">Loading matches...</p>;
  }

  if (!matches.length) {
    return <p className="tw-text-sm tw-text-gray-500">No match activity yet.</p>;
  }
    ...
    <div className="tw-space-y-2" aria-live="polite">
      {meta && (
        <p className="tw-text-xs tw-text-gray-500">
          Capacity {meta.activeMentees ?? 0}/{meta.capacity ?? '—'} · Remaining slots {meta.remainingSlots ?? '—'}
        </p>
      )}
      <ul className="tw-divide-y tw-divide-gray-200 tw-rounded-2xl tw-border tw-border-gray-100">
        {matches.map((match) => (
          <li key={match.id} className="tw-flex tw-justify-between tw-items-center tw-p-4 tw-gap-3">
            <div>
              <p className="tw-font-medium tw-text-gray-900">{match.mentee?.name}</p>
              <p className="tw-text-xs tw-text-gray-500">Score {match.score}</p>
            </div>
            <span className={`tw-text-xs tw-font-semibold tw-rounded-full tw-px-3 tw-py-1 ${statusColors[match.status] || 'tw-bg-gray-100 tw-text-gray-700'}`}>
